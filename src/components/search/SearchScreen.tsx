    ...
    <div className="flex flex-col w-full h-full bg-mt-cream overflow-hidden">
      <div className="flex items-center gap-3 px-4 pt-6 pb-3 flex-shrink-0">
        ...
        <div className="flex-1 flex items-center gap-2 bg-white rounded-2xl border border-mt-linen px-3 py-2">
          <Search size={16} className="text-mt-muted" />
          <input
            autoFocus
            value={query}
      ...
      <div className="flex-1 overflow-y-auto">
        {users.length > 0 && (
          <section>
            <h2 className="text-[10px] font-semibold text-mt-muted uppercase tracking-wide px-4 py-2">Pessoas</h2>
        ...
        {communities.length > 0 && (
          <section>
            <h2 className="text-[10px] font-semibold text-mt-muted uppercase tracking-wide px-4 py-2">Comunidades</h2>
            <ul className="divide-y divide-gray-100">
              {communities.map((c) => (
                <li key={c.id}>
                  <button onClick={() => onOpenCommunity(c.id)} className="w-full flex items-center gap-3 px-4 py-3 text-left">
                    <div className="w-10 h-10 rounded-2xl bg-mt-rose flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                      {c.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-mt-charcoal">{c.name}</p>
        ...
        {debouncedQuery.length >= 2 && users.length === 0 && communities.length === 0 && (
          <p className="text-sm text-mt-muted text-center py-8">Nada encontrado para "{debouncedQuery}"</p>
        )}
      </div>
    </div>
  );
