...
describe('DashboardScreen — Sara hero CTA', () => {
  it('shows "Conversar com a MãeIA" button', () => {
    useAppStore.setState({ isLoggedIn: true, motherName: 'Ana', phase: { stage: 'pregnant', week: 28 } })
    render(<DashboardScreen />, { wrapper: makeWrapper() })
    const saraButtons = screen.getAllByRole('button', { name: /conversar com a mãeia/i })
    expect(saraButtons.length).toBeGreaterThan(0)
  })

  it('"Conversar com a MãeIA" navigates to maeIA tab', () => {
    ...
    fireEvent.click(screen.getAllByRole('button', { name: /conversar com a mãeia/i })[0])
    expect(useAppStore.getState().activeTab).toBe('maeIA')
  })

  it('renders Sara FAB button', () => {
    render(<DashboardScreen />, { wrapper: makeWrapper() })
    // There may be multiple "Conversar com a MãeIA" elements (the card link + the FAB)
  ...
  it('Sara FAB opens MaeIA overlay with back button', () => {
    render(<DashboardScreen />, { wrapper: makeWrapper() })
    // The FAB has aria-label="Conversar com a MãeIA" and is type="button"
