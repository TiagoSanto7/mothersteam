...
const ICON = {
  like:    <Heart size={16} className="text-blush-500" />,
  follow:  <UserPlus size={16} className="text-lavender-600" />,
  comment: <MessageCircle size={16} className="text-sage-600" />,
};
      ...
      <div className="flex items-center justify-between px-4 pt-6 pb-4 border-b border-gray-100">
        ...
        {unreadCount > 0 ? (
          <button
            ...
            className="text-[11px] text-lavender-600 font-semibold"
          >
            Marcar lidas
          </button>
        ) : (
        ...
        {notifications.length === 0 ? (
          ...
          <ul className="divide-y divide-gray-100">
            {notifications.map((n) => (
              <li
                key={n.id}
                className={`flex items-start gap-3 px-4 py-4 ${!n.read ? 'bg-lavender-50' : 'bg-white'}`}
              >
                <div className="w-9 h-9 rounded-full bg-lavender-100 flex items-center justify-center flex-shrink-0">
                  {ICON[n.type]}
                </div>
                ...
                {!n.read && (
                  <div className="w-2 h-2 rounded-full bg-lavender-600 flex-shrink-0 mt-1.5" />
                )}
              </li>
            ))}
          </ul>
