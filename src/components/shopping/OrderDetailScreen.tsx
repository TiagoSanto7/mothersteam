...
export function OrderDetailScreen({ orderId, onBack }: Props) {
  ...
  if (!order) return (
    <div className="flex flex-col h-full bg-mt-gradient-pastel">
      ...
      <p className="text-center text-sm text-mt-muted mt-8">Pedido não encontrado.</p>
    </div>
  )
    ...
    <div className="flex flex-col h-full bg-mt-gradient-pastel overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-8">
        <div className="bg-white/60 rounded-3xl p-4 mb-4">
          ...
          {isCancelled ? (
            <div className="flex items-center gap-2 text-mt-rose-dark">
              <Circle size={16} />
                ...
                return (
                  <div key={status} className="flex items-center gap-3">
                    <div className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 ${
                      isDone ? 'bg-mt-rose' : 'bg-mt-muted/20'
                    }`}>
                      {isDone ? (
                        <CheckCircle size={12} className="text-white" />
                      ) : (
                        <Circle size={12} className="text-mt-muted/40" />
                      )}
                    </div>
                    <span className={`text-sm ${isCurrent ? 'font-semibold text-mt-charcoal' : isDone ? 'text-mt-muted' : 'text-mt-muted/40'}`}>
                      {STATUS_LABEL[status]}
                    </span>
                  </div>
                )
          ...
          {order.trackingCode && (
            <div className="mt-3 pt-3 border-t border-mt-linen/60">
              <div className="flex items-center gap-2 flex-wrap">
                <Truck size={14} className="text-mt-rose flex-shrink-0" />
                <span className="text-xs text-mt-muted">Rastreio:</span>
                <span className="text-xs font-mono font-semibold text-mt-charcoal">{order.trackingCode}</span>
                {/^[A-Z]{2}\d{9}[A-Z]{2}$/i.test(order.trackingCode) && (
                  <a
                    ...
                    className="text-[10px] text-mt-rose font-semibold underline underline-offset-2"
                  >
          ...
          {order.items.map((item) => {
            ...
            return (
              <div key={item.id} className="flex gap-3 mb-3 last:mb-0">
                {img ? (
                  <img src={img} alt={item.ownProduct.name} className="w-12 h-12 rounded-xl object-cover bg-mt-linen flex-shrink-0" />
                ) : (
                  <div className="w-12 h-12 rounded-xl bg-mt-linen flex items-center justify-center flex-shrink-0">
                    <Package size={14} className="text-mt-muted" />
                  </div>
                )}
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-mt-charcoal line-clamp-1">{item.ownProduct.name}</p>
                  <p className="text-xs text-mt-muted">
        ...
        <div className="bg-white/60 rounded-3xl p-4 mb-4">
          <div className="flex justify-between text-sm text-mt-charcoal mb-1.5">
            <span>Subtotal</span>
          ...
          <div className="border-t border-mt-linen pt-2 flex justify-between text-sm font-bold text-mt-charcoal">
            <span>Total</span>
            <span className="text-mt-rose">R$ {Number(order.total).toFixed(2)}</span>
          </div>
        </div>

        <div className="bg-white/60 rounded-3xl p-4">
          <p className="text-xs font-semibold text-mt-muted uppercase tracking-wide mb-2 flex items-center gap-1">
            <MapPin size={11} /> Endereço de entrega
          </p>
          <p className="text-sm font-medium text-mt-charcoal">{order.address.recipientName}</p>
          <p className="text-xs text-mt-muted">
          ...
          <p className="text-xs text-mt-muted">CEP {order.address.zipCode}</p>
        </div>
      </div>
    </div>
  )
