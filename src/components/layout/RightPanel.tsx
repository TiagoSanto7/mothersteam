...
export function RightPanel({ onOpenUser, onOpenCommunity }: RightPanelProps) {
  ...
  return (
    <div className="bg-mt-cream border-l border-mt-linen sticky top-0 h-screen overflow-y-auto p-4 flex flex-col gap-5">
      {/* Search bar */}
      <div className="relative">
        <div className="flex items-center gap-2 bg-white rounded-2xl border border-mt-linen px-3 py-2.5">
          <Search size={16} className="text-mt-muted flex-shrink-0" />
          <input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Buscar pessoas e comunidades"
            aria-label="Buscar pessoas e comunidades"
            className="flex-1 bg-transparent text-sm text-mt-charcoal placeholder:text-mt-muted outline-none min-w-0"
          />
        </div>

        {debouncedQuery.length >= 2 && (searchUsers.length > 0 || searchCommunities.length > 0) && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-2xl border border-mt-linen shadow-lg z-10 overflow-hidden max-h-72 overflow-y-auto">
            {searchUsers.length > 0 && (
              <section>
                <h3 className="text-[10px] font-semibold text-mt-muted uppercase tracking-wide px-3 py-2">Pessoas</h3>
                {searchUsers.map((u) => (
                  <button
                    key={u.id}
                    onClick={() => { setQuery(''); onOpenUser(u.id); }}
                    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-mt-linen transition-colors"
                  >
                    <div
                    ...
                    <span className="text-sm font-medium text-mt-charcoal truncate">{u.name}</span>
                  </button>
                ))}
              </section>
            )}
            ...
            {searchCommunities.length > 0 && (
              <section>
                <h3 className="text-[10px] font-semibold text-mt-muted uppercase tracking-wide px-3 py-2">Comunidades</h3>
                {searchCommunities.map((c) => (
                  <button
                    ...
                    className="w-full flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-mt-linen transition-colors"
                  >
                    <div className="w-8 h-8 rounded-xl bg-mt-rose flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                      {c.name.charAt(0).toUpperCase()}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-mt-charcoal truncate">{c.name}</p>
                      <p className="text-[11px] text-mt-muted">{c._count.members} membros</p>
                    </div>
                  </button>
                ))}
              </section>
        ...
        {debouncedQuery.length >= 2 && searchUsers.length === 0 && searchCommunities.length === 0 && (
          <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-2xl border border-mt-linen shadow-lg z-10 px-3 py-4">
            <p className="text-sm text-mt-muted text-center">Nada encontrado</p>
          </div>
        )}
      </div>
      ...
      {suggestedUsers.length > 0 && (
        <section>
          <h2 className="text-xs font-semibold text-mt-muted uppercase tracking-wide mb-3">Mães para seguir</h2>
                  ...
                  <button
                    ...
                    className={[
                      ...
                      isFollowed
                        ...
                        : 'bg-mt-rose text-white hover:bg-mt-rose/90',
                    ].join(' ')}
                  >
                    {isFollowed
      ...
      <section className="mt-auto">
        <h2 className="text-xs font-semibold text-mt-muted uppercase tracking-wide mb-2">Frase do dia</h2>
        <div className="bg-mt-rose/10 rounded-2xl px-4 py-3">
          <p className="text-sm font-medium text-mt-charcoal leading-relaxed">{dailyPhrase}</p>
        </div>
      </section>
    </div>
  );
